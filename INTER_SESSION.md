...
- **Dashboard.tsx**: removed the "N new" counter from the right-rail header per user ask — now just "RECENT ARTICLES". Live-verified in preview.
- **bluesky_firehose.py** (generalizable, any-campaign): `_build_keyword_set` no longer fragments compound place names — "Wilkes-Barre" stays whole (was splitting into "wilkes"+"barre"; "barre" = 119 false hits / 0% precision in prod). `_post_matches` is now whole-word (compiled `\b(...)\b`, lru-cached) instead of substring, so "barre" can't fire inside "barrel"/"disbarred"/"RJ Barrett". Env hatches preserved. Tests rewritten + whole-word matcher test → 16/16.
- **#2 Provenance safety net** (ingestion.py — `_campaign_people` / `_district_label_variants` / `_provenance_rescue_label` / `_apply_provenance_rescue`): when a monitor named for a *specific* race participant pulls an item but the body is too thin to judge (< 300 chars, env `PROVENANCE_RESCUE_MAX_BODY_CHARS`), route it to the **review queue** (`reviewed=False, archived=False, content_category="campaign", actionability_label="review"`) instead of silently archiving. `actionability_label="review"` is the master key — it clears the review-queue score floor AND the keyword-gate bypass (verified by an integration test that runs `review_queue._review_queue_query` + `relevance_gate.passes_gate` on a rescued row). Hooked into both archive-commit branches (LLM-irrelevant + `_used_fallback`). 21 new tests, all green.
- **#1 Google-News publisher_domain derivation** (outlet_linking.py — `_normalize_outlet_name` / `build_outlet_name_domain_index` / `publisher_name_from_title` / `match_publisher_to_domain` / `derive_publisher_domain_from_title`; wired into ingestion.py `ingest_rss` right before `recover_body`). **Took Option 1B, not 1A.** The batchexecute redirect *decoder* is dead (Google removed `data-n-a-sg`/`data-n-a-ts`; empirically returns None for every real CBMi… ID) and reviving it is a brittle arms-race — so instead of decoding the redirect, we **unlock the body-recovery path that already exists**. `recover_body`'s title-search fallback needs a `publisher_domain` to query, but ~60% of Google-News entries carry no `<source href>`, leaving it NULL. Fix: derive the domain from the publisher label Google appends to every title ("`<Headline> - <Publisher>`") and map it against the **outlet catalog** (no PA hard-coding → generalizes to any NOCTUA tenant). That domain both unlocks the title-search body fetch *before the relevance gate* and improves outlet attribution/display. 19 new tests, all green.

### Key decisions / data-validation trail
- **Provenance signal is surname + a disambiguator, NOT bare surname.** Real-data check (read-only on live, 22,140 archived rows): bare-surname matching rescued 498 rows but ~half were homonyms — the *novelist Paolo Cognetti* ("le otto montagne"), the *1900s ballplayer Roger Bresnahan*, the *voice actress Alyssa Bresnahan*, a "Tampa Police Chase" YouTube clip. Read titles (not counts) to catch this. Tightened to **surname + (full first name OR district id OR a second race surname)** — all config-derived, no PA hard-coding. Result: **162 rescued, ~95%+ precision**; all 5 known homonym monitors (`Mastodon #Cognetti`, `YouTube: Bresnahan`, `Reddit via Google News: …`, `PR Newswire: …`, `National pickup: …`) correctly excluded. The 13 added by the district/two-surname branches ("Google News — PA-08 Cognetti Bresnahan") are 100% real (polling, launch coverage, stock-trade attacks).
- **#2 is going-forward ingestion only** — it does not move the 162 already-archived rows. That's exactly what **#3 (targeted recovery rescore)** is for. **#1 (publisher_domain derivation)** is the *primary* fix (fetch the real body before the gate so most never go thin); #2 is the backstop for resolution failures.
- Rescued state is deliberately honest: `race_relevance_score`/`label` keep the thin-text verdict (0/irrelevant); only category+actionability carry the provenance override. We override the ARCHIVE action, not the text score.
- **#1 matcher is exact + whole-name containment ONLY — never lone-word overlap.** Read-only validation on 1,569 live thin/null-domain Google-News redirect items: the loose any-shared-word strategy (`link_outlet_to_item`'s) mis-mapped `6abc Philadelphia` + `FOX 29 Philadelphia` → the NBC outlet (shared "philadelphia") and a `Funeral Home` → the "Homepage" outlet (shared "home"). Containment (hyphen/punct-normalized, ≥6-char outlet name) rejects all three while still catching `Hazleton Standard Speaker`→standardspeaker.com and `Scranton Times-Tribune`→thetimes-tribune.com. **Addressable: 1,357 / 1,569 (~86%)** map to a catalog domain at high precision. Honest caveat: a *derived* domain ≠ a *recovered* body — several top PA publishers (Citizens' Voice, Times-Tribune, Standard-Speaker) 403 at their WAF, so those gain attribution but not body text; the non-403 set (Times Leader, Pocono Record, Capital-Star, River Reporter, Spotlight PA, WITF, WHYY, …) gains both.

### #3 DONE — but RE-SCOPED to the free provenance rescue; the broad body-recovery sweep was REJECTED on a measured 9% yield
- **The decisive measurement (read-only, HTTP-only, no LLM, no writes): real body-recovery rate = 9% (4/45 sampled).** #1 now derives a `publisher_domain` for 2,065 of the 2,257 archived gnews-thin items, but a *derived domain ≠ a recovered body*: the top PA publishers (poconorecord, wnep, scrantontimes, pennlive, standardspeaker, citizensvoice, …) **403 at their WAF**, so `recover_body` rarely actually fetches text. A broad sweep would be 2,065 HTTP fetches → only ~183 real bodies → ~183 metered LLM rescores. Poor yield-per-effort, and it recovers mostly long-tail outlets, not the majors. **User chose to skip it.** (If revisited: gate the fetch to the known non-403 outlet set so effort isn't spent on WAF-blocked domains.)
- **What shipped instead — retroactive #2 provenance rescue over the whole archived thin backlog (FREE: no HTTP, no LLM, no rescore).** Reused the LIVE `ingestion._apply_provenance_rescue` *verbatim* (same homonym-safe predicate, same rescued state) rather than reinventing column writes. **162 items un-archived → review queue** (`archived=False, content_category="campaign", actionability_label="review", reviewed=False`): 136 Google-News + 26 non-gnews (21 Reddit-via-Tavily, 4 candidate-own X/Twitter, 1 other). The prior-session backlog estimate of 162 matched exactly. **Proof-of-outcome (ran the real `review_queue._review_queue_query` + `_partition_by_relevance`): 160 land in the queue and ALL pass the keyword gate — confirms the `actionability="review"` keyword-gate bypass works in practice (0 keyword-filtered).**
- **Correction applied post-write — the backlog script overreached on 2 previously-DISMISSED items.** The live `_apply_provenance_rescue` only ever runs on fresh ingests (never dismissed/reviewed), so it has no `dismissed` guard; my backlog application inherited that gap and un-archived 2 rows a human had already dismissed (id=3809 a candidate-own off-topic tweet mis-scored 80 → would have leaked into the *feed*, since the feed gate doesn't check `dismissed`; id=24966 score 0, harmless). **Re-archived both** to honor the prior human reject. Net: **160 in review, 37 feed-eligible.** Honest limitation: the rescue sets `reviewed=False` unconditionally, so I cannot post-hoc identify any rows that were human-reviewed-and-archived (vs auto-archived) — but the predicate targets *thin-body auto-archives* and the strongest human-reject signal (`dismissed`) was checked and honored. **Lesson for any future backlog-rescue: filter `dismissed=False AND reviewed=False` before applying a function written for the fresh-ingest path.**
- **Safety properties (all verified post-write):** `race_relevance_score` left UNTOUCHED on all 162 → **0 nulled scores → no `_resume_pipeline_if_needed` rescore fires on restart** (that's the only thing that would have cost LLM money). Function guards on `archived_as_irrelevant` → **idempotent** (re-run finds 0 qualifiers). **Reversible** by id (re-archive). Dry-run-first on both batches before commit.
- **Blast radius the user signed off on:** 37 of the surfaced items score ≥50, so un-archiving makes them **feed-eligible** (dashboard.py:320 gate), not just review-queue. That is the *correct* recall fix — these are genuinely-relevant campaign items that were wrongly archived on a thin body (e.g. id=24531 "SOUND THE ALARM: Mayor Paige Cognetti (PA-08) Slams Bresnahan" scored 90; an 84-scored Bresnahan-legislation item). They carry thin bodies, so a click shows a short stub, but the headline-level signal is right; story-clustering dedups the near-duplicates at display time. The rest are review-queue-only.
- Scripts (idempotent, dry-run default) kept at `/tmp/apply_fix3_provenance_rescue.py` (gnews) and `/tmp/apply_fix3_provenance_rescue_allsrc.py` (all-source). Affected-id lists printed in their output for audit/rollback.

### Open questions / concerns for review
- **Watch review-queue depth.** 162 just landed there in one shot; the *ongoing* #2 inflow should be much smaller now that #1 derives domains (most new gnews bodies will fetch before the gate). If the queue feels heavy, the 124 review-only rescues are the bulk — triage or bulk-dismiss.
- The 9% WAF reality means #1's value is mostly **outlet attribution/display**, not body recovery, for the major PA publishers — it still unlocks bodies for the non-403 long tail (Times Leader, Pocono Record, Capital-Star, River Reporter, Spotlight PA, WITF, WHYY). Worth a future note if anyone expects #1 to "fix the thin bodies" wholesale — it can't while the majors WAF-block.

---

## 2026-05-31 Session: Second improvement batch — auth/scheduler fixes + source_items indexes (SAFE migration, no incident this time)

Continuation of the review above. User: "go through them one by one and implement the fixes… verify these are actually bugs and won't break anything. If confident, proceed." Same outcome shape as before — a few real safe wins, several "bugs" that were actually intentional.

### Built (safe, shipped — all uncommitted)
- **main.py access_code_guard**: removed the `?access_code=` query-param auth fallback (kept the `X-Access-Code` header). Verified no frontend `?access_code=` usage and **no SSE/StreamingResponse endpoints** (the only thing that couldn't send a header). Stops codes leaking into server logs / browser history / referer. Live-verified: tunnel-host request with a query code → 401.
- **scheduler.py**: `AsyncIOScheduler(job_defaults={"misfire_grace_time": 3600})`. APScheduler's default is **1s**, so every interval job (ingestion/crawler/reddit/…) + both one-shot `date` rematch jobs were silently dropped whenever the laptop slept, until the next interval. Now the latest missed run fires once on wake; per-job `coalesce=True` still collapses backlogs. Safe across all jobs (all are interval or idempotent date jobs — no wall-clock cron).
- **source_items indexes** (migration `56afe3d07226`, down_revision `84a1922627dd`): `ix_source_items_published_at`, `ix_source_items_source_url`, `ix_source_items_outlet_id`. `index=True` added to all three on `SourceItem` in models.py.
  - **Did it the safe way this time** (vs the incident above): built on live first with `CREATE INDEX CONCURRENTLY IF NOT EXISTS` (non-blocking), verified `indisvalid=t`, THEN wrote the migration as a complete file (full body, never a stub) using plain `CREATE INDEX IF NOT EXISTS` so the `--reload` auto-apply was a true **no-op** on live. Validated up→down→up on `noctua_scratch` via `alembic -x url=…`. Live `alembic_version` = `56afe3d07226`, backend healthy.

### EXPLAIN-verified impact (the honest version)
- **source_url**: was a seq scan (~4600 cost) on every feed-entry dedup; now **Index Scan** (~8). Clear win.
- **published_at**: plain `ORDER BY published_at DESC` (dashboard.py:323, sources.py:29, entities.py) now **Index Scan Backward** (~5422 → ~33). BUT the **6 `.desc().nullslast()` sites** (narrative_frames.py:143/2356/2424, narrative_landscape_established.py:357, entity_network[hidden], a script) are **NOT** served — a plain btree can't satisfy `DESC NULLS LAST` (only ~547/24,740 rows are NULL). Left as plain `index=True` (repo convention); a dedicated `(published_at DESC NULLS LAST)` index is a possible follow-up if those frame-list paths prove slow — not added speculatively (extra write cost on the hot ingestion path).
- **outlet_id**: built; planner uses it for outlet-selective/count paths (for filtered+ordered+LIMIT it prefers the published_at index — expected).

### Verify-first triage (NOT changed — intentional or not worth it)
- **Cerebras default model** (llm_provider.py:1266/1519): `.env` sets `CEREBRAS_MODEL=gpt-oss-120b` **explicitly**, matching the factory default — deliberate. The class default `qwen-3-235b-a22b-instruct-2507` is just dead in the factory path. "Fixing" it would have been a regression. Not a bug.
- **Remove dead SQLite/KG code** (search_index.py SQLite path; entity_network/entity_review routes): CLAUDE.md says **intentionally retained until Phase 5 (~2026-06-26)**; rollback artifacts depend on the SQLite path. Holding.
- **Hoist `_build_context` per-article reads** (campaign_analysis.py:969): the 2 reads are tiny + cached and dwarfed ~1000× by the LLM call in the same function. Not worth a hot-path refactor. (Its real value — the unindexed `source_url` dedup — is the `source_url` index above.)
- **Disable Gemini thinking-mode**: Gemini is only a deep fallback (after Cerebras+Groq exhaust); disabling means injecting provider-specific params into the live OpenAI-compat call path (break risk) for a quality/cost tradeoff. Not a clear bug. Holding.

### Open (need user involvement)
- **#20 analytics N+1 / count endpoints** (analytics.py:200-218, dashboard.py:362-370, review_queue.py:131-133): only safe if provably result-equivalent — not yet verified.
- **#21 X-Forwarded-Host hardening + auth rate-limit**: needs a Vite restart (user's call per the tunnel rule) + a shared secret-header design; rate-limit is awkward because all traffic appears as localhost behind the Vite proxy.
- All edits this batch are **uncommitted** (on top of prior-session commit `6818d06`). Commit decision left to user.
...
## 2026-05-31 Session: Briefing v2 — fix citation that pointed at a contradictory quote
### The bug (Theo's report)
The grounded memo led with "Rob Bresnahan's efforts to secure federal freeze relief for PA growers[1]... rising from 0 to 12 mentions this week," but footnote [1] quoted an article that said the *opposite* — a union endorsing Cognetti because "Bresnahan broke his promise to protect health care by voting for historic cuts to Medicaid." The citation flatly contradicted the sentence it sourced.
### Root cause (diagnosed read-only, no writes/LLM)
The memo's numbers (0→12, 20→1) are REAL `frame_pulse_counts` deltas and the freeze-relief development is real (article #21533). But the v2 prompt fuses THREE disjoint pools — RECENT ARTICLES, NARRATIVE MOMENTUM (counts), RECENT QUOTES (`top_claims_for_briefing`, the ONLY pool carrying `[CN]` markers). The freeze story had ZERO matching quote in the citable pool, so the LLM, told to cite, grabbed the nearest quote that merely *named* Bresnahan — the Medicaid attack — to "ground" the lead. The analysis was sound; only the evidence-wiring was fabricated.
### Built (all in `services/briefing_summary.py` + new test)
- **Prompt rewrite — two statement classes, cited differently.** CITATION RULES now distinguishes (1) MOMENTUM/COUNT statements (from NARRATIVE MOMENTUM — **NEVER** get a `[CN]`, stated plainly uncited) from (2) WHAT-SOMEONE-SAID/DID statements (must be a verbatim RECENT QUOTES line + `[CN]`). Explicitly: **the lead MAY be uncited** ("an uncited true sentence ALWAYS beats a cited misleading one"); a shared NAME is not topical support; don't manufacture citations to hit a count. Also fixed the GOOD/BAD bridge examples — they previously cited count clauses, *teaching* the exact bug.
- **Deterministic guardrail (the real backstop — prompt-only enforcement is unreliable, same lesson as `_strip_recency_fillers`).** New `_citation_supported(sentence, quote, names)` strips any `[CN]` whose quote shares ZERO substantive topic words with the sentence it sits in. `_topic_tokens` = lowercase alpha ≥4 chars, minus stopwords, minus the claim's own entity names AND the race's principal subject names. `_sentence_for_marker` pulls the local sentence around each marker. Wired into `_run_grounded_llm`'s post-process: invalid (hallucinated) markers + off-topic markers are stripped from both the prose and the `citations[]` list; whitespace/punctuation re-tightened.
- **Two traps closed in the overlap test:** (a) "support" appears in BOTH "farmer-**support** narrative" and "throwing its **support** behind Cognetti" — added a discourse/process vocabulary group to `_CITATION_STOPWORDS` (support/narrative/mentions/push/boost/surge/fade/etc.) so it can't bridge unrelated topics. (b) a terse valid cite ("Cognetti's PRO Act commitment to protect workers' rights[C2]") must survive — so `subject_names` (candidate + opponents, built in `get_or_generate_grounded` from `campaign`+`opponents`) are excluded from the overlap test, and a <2-topic-token sentence/quote defers to the prompt (returns supported, never strips on thin signal).
### Key decisions
- **Strip the citation, not the sentence.** When the lead has no backing quote, the guardrail removes only the `[CN]` — the true sentence + its count stay. Matches Theo's chosen approach ("state it, no fake footnote + safety net").
- **Conservative by construction — essentially never false-strips a valid cite.** Stripping requires content-rich (≥2 topic tokens each side) AND zero intersection. Adding words to the stopword list can only *reduce* stripping.
### Verified
- New `tests/test_briefing_citation_guardrail.py` — **13 tests, all pass** (no LLM, no DB). Includes the exact freeze-vs-Medicaid miscite end-to-end through `_run_grounded_llm` with a mock LLM: `[C1]` (Medicaid quote on the freeze sentence) stripped from prose + `citations[]`, `[C2]` (PRO Act, genuinely on-topic) kept; invented `[C9]` stripped; uncited-lead memo left untouched.
- `tests/test_briefing_retrieval.py` still green (16 tests). 29 total pass.
- Public signature of `get_or_generate_grounded` unchanged (dashboard.py:397 call site unaffected); `_run_grounded_llm`'s new `subject_names` param defaults to None.
### For Theo / next session
- **No server restart needed.** The prompt text changed → input hash changes → the v2 cache misses → next `/briefing/morning?v=2` load regenerates with the new prompt + guardrail (one LLM call, then re-cached). `--reload` already picked up the file edit. Just reload the Briefing page; first load takes a few seconds.
- The guardrail is lexical (topic-word overlap), not semantic — it catches the *gross* mismatch (different topic entirely), which is the failure mode that happened. A quote on the same topic but wrong stance would still pass the overlap test; the prompt rules are the only defense there. Worth a ChatGPT second-opinion on the prompt rewrite if we want to harden the stance case.

## 2026-05-31 Session: Briefing v2 — restructure the LLM input so miscites are unrepresentable (Phase 3, follow-on to the guardrail above)
### Built
- Moved the citation-correctness work UPSTREAM of the LLM. The guardrail above is reactive (strip a bad cite after the model forms it); this layer is preventive (never hand the model a loose pile of quotes it can misattribute). Both layers now ship together and are independent.
- `briefing_summary._render_momentum_quotes(frames_meta, counts, best_frame, top_claims) -> (block, lead_line)` — PURE renderer (no DB, no LLM, deterministic). Groups each citable quote UNDER the narrative frame its article belongs to, keeping the quote's canonical 1-based `[CN]` index (the number `_run_grounded_llm` maps back by). A frame with momentum but no quote of its own renders `quotes: (none — state this development plainly, attach NO [CN])`. Quotes tied to no tracked frame fall into an `[OTHER]` bucket. Frames ordered by `(-abs(this_week - last_week), frame_id)`; `lead_line` = the largest-|delta| frame, computed in CODE not by the model.
- `briefing_summary._frames_for_articles(db, article_ids)` — maps article_id → (frame_id, name, owner_type, confidence) for the single highest-confidence ACTIVE `NarrativeFrameMention`; ties break to lowest frame_id. Same article→frame source the Dashboard's `_cited_frames_from_memo` already uses.
- `briefing_summary._momentum_and_quotes_block(db, top_claims)` — thin DB wrapper: gather active frames + `frame_pulse_counts` + `_frames_for_articles`, then call the pure renderer.
- Rewired `_build_grounded_prompt`: replaced the separate `_narrative_pulse_block` + numbered claims list with the ONE fused block + a "How to read this block" orientation paragraph + a "LIKELY LEAD: {lead_line}" line. Kept the literal phrases "NARRATIVE MOMENTUM" and "RECENT QUOTES" alive so every downstream HEADLINE/GROUNDING/CITATION rule reference still resolves. `_narrative_pulse_block` left untouched — v1 `_generate` still uses it.
- Tests: `tests/test_briefing_momentum_quotes.py` (14 — 9 pure-renderer + 5 SQLite article→frame). Full briefing suite green: 43 passed (momentum + guardrail + retrieval).
### Key decisions
- Grouping is a SOFT steer (only as good as frame-matching); the lexical guardrail remains the HARD backstop. Deliberately did not make correctness depend on frame-match accuracy — a misgrouped quote degrades to the `[OTHER]` bucket (cite-able only for "what X said/did"), which is the safe failure, not a miscite.
- Verified on the REAL DB (read-only render, no LLM): the ↑12 "Bresnahan Supports Local Farmers" frame — the exact freeze-relief surge from the bug — now shows `(none)`, and the contradictory Medicaid quote sits under "Bresnahan's Record on Medicaid Cuts," structurally unavailable to source the farmer-support lead. This week the lead is actually "Cognetti's Anti-Corruption" (↓19, a bigger swing); it too is quoteless and renders `(none)`.
- Determinism preserved (the input-hash cache depends on it): byte-stable block for stable inputs — frames by |delta| then id, quotes by index. Pinned by a test.
### For Theo / next session
- Same as the guardrail entry: no restart needed. Prompt text changed → input hash changes → next `/briefing/morning?v=2` regenerates once and re-caches. Reload the Briefing page.
- The lead is now picked in code (largest momentum swing), but the prompt still lets the model override it "if a single dominant event in the quotes is bigger." If we ever want the lead locked, remove that escape clause — but it's there on purpose so a huge one-off (endorsement, indictment) can outrank a pure count swing.

## 2026-05-31 Session: Headline-promotion backfill — surface obviously-relevant articles stuck in review/archive
### The complaint (Theo's report)
"Looking at the review queue right now, some are obviously relevant. The review queue should be *ambiguous* articles — in the top 5 pending review, 1 directly mentions Paige Cognetti and 3 directly reference Rob Bresnahan." His read: a deeper scoring/data issue, not a one-off.
### Root cause (diagnosed read-only via `/tmp/why_blocked.py`, `/tmp/diag_review_queue.py`)
**Not the scorer — a content_category contradiction.** 70 of the 79 in-review headline-match items were scored **79–95** yet carried `content_category="irrelevant"`. The feed gate (`dashboard.py:315`) requires `content_category != "irrelevant"`, so a 90-scored article naming the candidate in its headline was *gated out of the feed by its category* while still clearing the review-queue's `score>=40` gate — landing it in triage instead of the feed. Most of these were items an earlier **provenance rescue** un-archived *to review* but never re-categorized, leaving the score/category contradiction the `review_queue.py:38-47` comment already flagged.
### Built
- **Standing go-forward rule** (`ingestion.py` `_apply_headline_feed_promotion`, runs on every fresh ingest): when a thin-body item's HEADLINE names the race **homonym-safely** (full name / district label / two distinct surnames IN THE TITLE, **or** a candidate-named monitor as provenance — `_headline_names_race_disambiguated`), it: un-archives, normalizes `content_category` (None/irrelevant→`campaign`), floors `race_relevance_score` to the feed cutoff (`_HEADLINE_FEED_FLOOR`, env `HEADLINE_FEED_FLOOR=50`), and **marks `reviewed=True`/`dismissed=False`** so it surfaces in the feed WITHOUT inflating triage. Homonym-safety is config-derived (`_campaign_people`, `_district_label_variants`) — generalizes to any campaign's names, no PA-08 hard-coding.
- **One-time backlog pass** (`/tmp/backfill_headline_promotion.py`, `--apply --include-archived`): reused the LIVE helper VERBATIM so the one-shot pass and the standing rule can never diverge. Touched **239 rows** = 79 in-review (MIN: re-categorize + score-floor, no un-archive) + 160 archived (BONUS: un-archive). **4 profile/utility pages excluded** (ids 16/139/3/13296).
### Key decisions
- **Sentiment one-liners are KEPT; only utility/profile pages excluded** — Theo's correction: *"the troll tweet is signal isn't it? The profiles aren't but that tweet shows something."* A "nasty greedy troll" / "scared of ghosts" / "Support Paige Cognetti" social post IS a narrative signal for a narrative-intelligence tool; a House.gov contact page or an org bio-listing is not. Excluded ids are the 4 non-article pages only (16=Bresnahan House.gov contact, 139=NewDEAL bio, 3 & 13296=LCV profile listings).
- **Score-floor mechanism, no migration, fully reversible.** `HEADLINE_FEED_FLOOR` env-configurable; full BEFORE-state snapshots written for exact rollback (archived + content_category + score + label + reviewed/dismissed). No rescore (would be ~177h over 21k rows), no LLM calls — pure data normalization.
### Regression caught & fixed (post-write verification, did NOT trust "COMMITTED")
- First commit (un-archive + irrelevant→campaign) made 105 of the 239 promoted items pass the **review-queue** category gate → review depth INFLATED 2→107, the OPPOSITE of Theo's goal. `/tmp/verify_after.py` caught it. Fixed by adding `reviewed=True`/`dismissed=False` to the live helper (the key difference from the provenance rescue: that routes weaker evidence TO review for a glance; this is confident enough to SKIP review) and running `/tmp/fix_reviewed_flag.py --apply` over the 239.
### Verified (`/tmp/verify_final.py`, read-only, re-confirmed at session resume)
- **FEED 804 → 976 (+172 surfaced).  REVIEW 107 → 2  (0 promoted items remain in review).** The 2 left are genuinely ambiguous (24895 "PA lawmakers introduce bills…" sc=45; 24347 "Office of Economic Development" sc=45).
- All 239 invariants clean: archived=False 239/239, cat!=irrelevant 239/239, score>=50 239/239, reviewed=True 239/239, dismissed=False 239/239; in-feed 172/239 (the other 67 lack `published_at` or fail `_is_llm_scored`). 4 excluded pages still archived.
- Reversibility artifacts: `/tmp/headline_promotion_before_state.PRE-APPLY.json` + `/tmp/headline_promotion_flags_snapshot.json`.
### Open questions / concerns for review
